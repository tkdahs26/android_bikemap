...
서울, 광양 등의 공공자전거대여소위치를 찾고,Places API를 연동하여 주변 환경 사진을 제공하는 어플입니다.


기능: 서울, 광양 등 선택된 도시에 따라 지도에 해당 지역의 자전거 대여소 또는 클릭 지점 위치를 마커로 표시.

MarkerOptions api을 활용하여 클릭위치에 마커 생성합니다.

Google Places API 연동
해당 위치의 주변 환경 사진 목록을 제공.

 google Places API를 이용해 장소의  길거리 뷰,정보,Photodata를 얻고 Photodata를 사진 배열 형식으로 구현했습니다
